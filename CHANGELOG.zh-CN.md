...
- **吸顶标题指示器与章节折叠**：长文档滚动时在顶栏下方优雅吸顶显示当前章节并支持一键回滚到章节起点；标题左侧支持交互式折叠/展开按钮，折叠内容时不破坏文档数据。
- **表格换行模式（`epytor.tableWrapMode`）**：支持配置单元格自动换行（`"wrap"`）或单行横向滚动（`"nowrap"`），并支持单元格内 `Shift + Enter` 软换行。
- **文本对齐工具（左对齐、居中、右对齐、两端对齐）**：支持块级对齐与实时渲染，集成于 TopBar 与浮动工具栏。
- **Mermaid 2.0 图表引擎**：
