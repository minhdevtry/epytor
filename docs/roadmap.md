...
* [ ] **下拉补全重复** — `pathComplete` / `imgPathComplete` / `imagePicker` → 提取 `createDropdown()` 到 `ui/dom.ts`
* [ ] **确认/取消编辑重复** — `startCaptionEdit` / `startSrcEdit` → 提取通用内联编辑
* [ ] **hover 弹出菜单重复** — `selectionToolbar` 格式下拉 / 对齐下拉 → 提取 `createHoverMenu()` 到 `ui/dom.ts`
* [ ] **空 catch 块**（12 处）— 至少 `console.warn`
